...
{
    public static async Task AddAnnotation(this CustomMap customMap)
    {
        var mapHandler = (IMapHandler?)customMap.Map?.Handler;
        var googleMap = mapHandler?.Map;
        if (mapHandler is not null && googleMap is not null && customMap.Map is not null)
        {
            googleMap.SetMapStyle(new MapStyleOptions(customMap.MapStyle?.JsonStyle.ToString()));
        }
    ...
    public static async Task AddAnnotation(this CustomPin customPin)
    {
        ...
        if (mapHandler is not null && googleMap is not null && customPin.Map is not null)
        {
            ...
            using var stream = a.GetManifestResourceStream("MauiMaps.light_map_theme.json");
